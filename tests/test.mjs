...
test("hpssFrame: 持続音は調波成分へ、瞬間的な広帯域音は打撃成分へ分かれる", () => {
  const bins = 64, frames = 21;
  const mags = [];
  for (let t = 0; t < frames; t++) {
    const m = new Float32Array(bins).fill(0.01);
    m[10] = 1.0;                     // 全フレームで持続するトーン（bin 10）
    if (t === 10) m.fill(0.8);       // フレーム10だけ広帯域の打撃
    if (t === 10) m[10] = 1.0;
    mags.push(m);
  }
  const mid = P.hpssFrame(mags, 10, 6, 6);
  // 打撃フレーム中のトーンは一部打撃側へ配分されるが、優勢は調波側のまま
  assert.ok(mid.harm[10] > 0.5 && mid.harm[10] > mid.perc[10],
    `トーンは調波成分が優勢 (${mid.harm[10]})`);
  assert.ok(mid.perc[30] > 0.6, `打撃は打撃成分へ (${mid.perc[30]})`);
  assert.ok(mid.harm[30] < 0.2, `打撃が調波成分を汚さない (${mid.harm[30]})`);
  const steady = P.hpssFrame(mags, 5, 6, 6);
  assert.ok(steady.harm[10] > 0.8, `通常フレームのトーン (${steady.harm[10]})`);
  assert.ok(steady.perc[10] < 0.2, `トーンが打撃成分に漏れない (${steady.perc[10]})`);
});
...
test("統合: ドラム入りのミックスでもメロディが崩れない（HPSS）", () => {
  const rate = 22050;
  const melodyNotes = [
    { start: 0.0, dur: 0.35, midi: 76, vel: 1 },
    { start: 0.4, dur: 0.35, midi: 79, vel: 1 },
    { start: 0.8, dur: 0.35, midi: 83, vel: 1 },
    { start: 1.2, dur: 0.35, midi: 79, vel: 1 },
  ];
  const drumHits = [];
  for (let t = 0; t < 1.6; t += 0.2) {
    drumHits.push({ t, type: t % 0.4 < 0.1 ? "kick" : "snare", strength: 1 });
  }
  const audio = P.mixFloat32([
    P.renderChip(melodyNotes, { wave: "square50", vibrato: 0, decay: 0 }, rate),
    P.renderDrums(drumHits, rate),
  ], [0.6, 0.55]);
  ...
  const melMidis = result.melody.map((n) => n.midi);
  for (const m of [76, 79, 83]) {
    assert.ok(melMidis.includes(m), `ドラム入りでもメロディ ${m}（実際: ${melMidis.join(",")}）`);
  }
  assert.ok(result.drums.length >= 4, `ドラムも検出される（実際: ${result.drums.length}）`);
});
